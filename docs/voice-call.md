---
summary: "Voice Call plugin: outbound + inbound calls via Twilio/Telnyx/Plivo/Asterisk (plugin install + config + CLI)"
read_when:
...
Asterisk support uses ARI for call control and WebSocket events. In end-to-end
Realtime mode, audio flows through Asterisk ExternalMedia RTP into the OpenAI
Realtime session and back to the phone call.

For a single Asterisk:
...
Use `asterisks[]` for multi-cluster routing. OpenClaw picks the Asterisk cluster
by longest phone-prefix match after normalizing a leading `8` to `7` for
11-digit numbers.
...
### Asterisk inbound profiles

`inboundGreeting` still works as the fallback greeting. For Asterisk, use
`inboundProfiles` when inbound callers need different greetings or Realtime
system prompts:

```json5
{
  provider: "asterisk",
  inboundGreeting: "Hello, this is the voice assistant. How can I help?",
  asterisk: {
    inboundProfiles: {
      defaultGreeting: "Hello, this is the voice assistant. How can I help?",
      defaultSystemPrompt: "You answer inbound calls briefly and collect the caller's message.",
...
The same `inboundProfiles` object can be placed on each entry in `asterisks[]`.
