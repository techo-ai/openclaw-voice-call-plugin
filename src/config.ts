...
export const AsteriskInboundProfileOverrideSchema = z
  .object({
    /** Caller numbers that should use this profile. Matching uses provider normalization. */
    callerNumbers: z.array(z.string().min(1)).min(1),
    /** Verbatim line spoken when the inbound call is answered. */
    greeting: z.string().min(1).optional(),
    /** Realtime system prompt used for this caller profile. */
...
export const AsteriskInboundProfilesConfigSchema = z
  .object({
    /** Default greeting for inbound Asterisk calls. Falls back to inboundGreeting. */
    defaultGreeting: z.string().min(1).optional(),
    /** Default Realtime system prompt for inbound Asterisk calls. */
    defaultSystemPrompt: z.string().min(1).optional(),
    /** Caller-number-specific profile overrides. */
...
export const AsteriskConfigSchema = z
  .object({
    ...
    ariUrl: z.string().min(1).optional(),
    /** ARI username */
    ariUsername: z.string().min(1).optional(),
    /** ARI password */
    ariPassword: z.string().min(1).optional(),
    /** Stasis application name (default: openclaw) */
    stasisApp: z.string().min(1).optional(),
    /** SIP context for originating calls (default: from-internal) */
    ...
    audioUploadUrl: z.string().optional(),
    /** Auth token for the audio upload server */
    audioUploadToken: z.string().optional(),
    /** System prompt passed to the OpenAI Realtime session during Asterisk calls. */
    ...
    /**
     * Logical cluster name. Required when several Asterisks are configured via
     * `asterisks[]` so OpenClaw can pick which one to dial out through. Ignored
     * in the single-cluster case.
     */
    name: z.string().optional(),
    /**
     * Phone-number prefixes this cluster accepts for outbound calls. Numbers
     * starting with one of these (after stripping leading + or 8→7 normalisation)
     * are routed to this cluster.
     */
    routePrefixes: z.array(z.string()).optional(),
    /** Use this cluster when no prefix matches. Exactly one default per list. */
...
export const VoiceCallConfigSchema = z
  .object({
    ...
    /** Asterisk ARI configuration (single cluster). */
    asterisk: AsteriskConfigSchema.optional(),

    /**
     * Asterisk ARI configurations (multi-cluster). When set, OpenClaw routes
     * outbound calls by phone-number prefix to the matching cluster. Useful
     * when one box has Tele2 trunk in KZ and another has Telfin in RU: a
     * single OpenClaw drives both.
     */
    asterisks: z.array(AsteriskConfigSchema).optional(),
...
export function validateProviderConfig(config: VoiceCallConfig): {
  ...
  if (config.provider === "asterisk") {
    ...
    // Multi-cluster mode: each entry validated independently, single asterisk field optional.
    if (clusters.length > 0) {
      clusters.forEach((c, i) => {
        const prefix = `plugins.entries.voice-call.config.asterisks[${i}]`;
        if (!c.ariUrl) errors.push(`${prefix}.ariUrl is required`);
        if (!c.ariUsername) errors.push(`${prefix}.ariUsername is required`);
        if (!c.ariPassword) errors.push(`${prefix}.ariPassword is required`);
      });
    } else {
      if (!config.asterisk?.ariUrl) {
        errors.push(
          "plugins.entries.voice-call.config.asterisk.ariUrl is required (or set ASTERISK_ARI_URL env)",
        );
      }
      if (!config.asterisk?.ariUsername) {
        errors.push(
          "plugins.entries.voice-call.config.asterisk.ariUsername is required (or set ASTERISK_ARI_USERNAME env)",
        );
      }
      if (!config.asterisk?.ariPassword) {
        errors.push(
          "plugins.entries.voice-call.config.asterisk.ariPassword is required (or set ASTERISK_ARI_PASSWORD env)",
        );
      }
    }
  }
