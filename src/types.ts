...
export const NormalizedEventSchema = z.discriminatedUnion("type", [
  ...
  BaseEventSchema.extend({
    /**
     * Transcript of something the AI caller said on the line. Emitted by
     * providers that run an embedded live voice agent (Asterisk + OpenAI
     * Realtime) so the call transcript reflects both sides of the conversation.
     */
    type: z.literal("call.bot_speech"),
    transcript: z.string(),
  }),
  BaseEventSchema.extend({
...
export type InitiateCallInput = {
  ...
  inlineTwiml?: string;
  /**
   * Task prompt for providers that can run a live OpenAI Realtime session
   * end-to-end (currently Asterisk). When set, the provider configures the
   * Realtime session with these instructions so the AI acts as the caller
   * toward the remote party for the duration of the call.
   */
  realtimeTaskInstructions?: string;
};
