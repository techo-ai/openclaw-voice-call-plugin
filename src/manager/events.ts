...
export function processEvent(ctx: EventContext, event: NormalizedEvent): void {
  ...
  // Fallback: when the provider's internal callId diverges from the manager's
  // internal callId — happens for inbound calls where the AsteriskProvider
  // invents its own UUID at StasisStart before createWebhookCall mints the
  // canonical one — every subsequent event for that call still carries the
  // provider-side callId. Without this fallback, `findCall` returns
  // undefined for everything except the very first event, so call.ended
  // / call.bot_speech / call.speech all silently drop and finalizeCall
  // never runs. Look up by providerCallId as a second chance so the call
  // record actually receives its lifecycle events.
  if (!call && event.providerCallId) {
    ...
    if (call) {
      // Normalize for downstream consumers that key off callId.
  ...
  switch (event.type) {
    ...
    case "call.bot_speech":
      // Provider-emitted bot transcript (live voice agent said something to the
      // callee). Record it so the call transcript reflects both sides of the
      // conversation — callers inspecting the transcript after a call see what
      // the agent actually spoke, not only what the callee replied.
