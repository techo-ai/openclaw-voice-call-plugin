...
export interface VoiceCallProvider {
  ...
  /**
   * Optional: return true if the provider is running an embedded live voice
   * agent (e.g. OpenAI Realtime) for this call. When true, the call manager
   * skips its own speak() TTS and transcript-writing — the live agent already
   * spoke and the provider emits its own call.bot_speech events.
   */
