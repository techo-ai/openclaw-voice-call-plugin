...
export class CallManager {
  ...
  constructor(
    ...
    storePath?: string,
    opts?: { onCallFinalized?: (call: CallRecord) => void },
  ) {
    this.config = config;
    this.storePath = resolveDefaultStoreBase(config, storePath);
    this.onCallFinalized = opts?.onCallFinalized;
  }
  ...
  private getContext(): CallManagerContext {
    return {
      ...
      onCallFinalized: this.onCallFinalized,
    };
  }
  ...
  /**
   * Check if a call has an active transcript waiter (i.e., continueCall is waiting).
   */
  hasActiveWaiter(callId: CallId): boolean {
    return this.transcriptWaiters.has(callId) || this.activeTurnCalls.has(callId);
  }
