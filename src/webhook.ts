    ...
    return new Promise((resolve, reject) => {
      ...
      this.server.on("error", (err: NodeJS.ErrnoException) => {
        if (err.code === "EADDRINUSE") {
          // Another instance (e.g. the gateway) already owns this port.
          // For providers like Asterisk that use WebSocket (ARI) for events,
          // the webhook server is not required.  Resolve with the expected URL
          // so the runtime can proceed without a local HTTP listener.
          const fallbackUrl = this.resolveListeningUrl(bind, webhookPath);
          this.listeningUrl = fallbackUrl;
          console.warn(
            `[voice-call] Port ${port} already in use — reusing existing webhook at ${fallbackUrl}`,
          );
          resolve(fallbackUrl);
          return;
        }
...
<Response>
   ...
   */
  async handleInboundResponse(callId: string, userMessage: string): Promise<void> {
